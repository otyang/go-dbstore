...
func TestSeeder_CreateTables(t *testing.T) {
	...
	err := NewSeeder(db).CreateTables(ctx, mmodels, imodels)
	assert.NoError(t, err)
...
func TestSeeder_DropTables(t *testing.T) {
	...
	err = NewSeeder(db).DropTables(ctx, mmodels, imodels)
	assert.NoError(t, err)
}
...
func TestSeeder_DropAndCreateTables(t *testing.T) {
	...
	err = NewSeeder(db).DropAndCreateTables(ctx, mmodels, imodels)
	assert.NoError(t, err)
}
...
func TestSeeder_CreateIndex(t *testing.T) {
	...
	seederdb := NewSeeder(db)

	err := seederdb.CreateTables(ctx, []any{(*Movies)(nil)}, nil)
	assert.NoError(t, err)

	err = seederdb.CreateIndex(ctx, (*Movies)(nil), "animal_id_index", "animal_id_index")
	assert.NoError(t, err)
}
